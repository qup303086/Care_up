    ...
    public class OrdersController : ApiController
    {
        ...
        public IHttpActionResult MemberOrderStatus(int id)//家屬id
        {
            ...
            var order5 = orderID.Where(x => x.Star != null)
                 .Where(x => x.Status == OrderType.已取消 || x.Status == OrderType.已完成
                                                       || x.Status == OrderType.家屬中斷 || x.Status == OrderType.照服員中斷
                                                       || x.Status == OrderType.待退款 || x.Status == OrderType.照服員拒接)
                 .ToList();
        ...
        public IHttpActionResult MemberGet10(int id)
        {
            ...
            db.SaveChanges();
            var orders = order.OrderByDescending(x => x.StartDate).Select(x => new
            {
                x,
        ...
        public IHttpActionResult MemberGet13(int id)
        {
            List<Orders> orders = db.Orders.Where(x => x.Elders.MemberId == id && x.Star == null)
                .Where(x => x.Status == OrderType.已完成 || x.Status == OrderType.照服員待收款
                            || x.Status ==OrderType.家屬中斷 || x.Status == OrderType.照服員中斷
                            ).ToList();
            if (orders.Count == 0)
            {
            ...
            var order = orders.OrderByDescending(x => x.StartDate).Select(x => new
            {
                x,
        ...
        [Route("MemberInterruptOrder")]
        [HttpPatch]
        public IHttpActionResult InterruptOrderM(OrderReject orderReject)
        {
            if (!string.IsNullOrWhiteSpace(orderReject.Cancel))
            {
                Orders order = db.Orders.Find(orderReject.Id);
                order.Cancel = orderReject.Cancel;
                order.Status = OrderType.家屬中斷;
                order.EditDate = DateTime.Now;
                db.SaveChanges();
                return Ok(new { message = "進行中訂單已由家屬中斷" });
            }
            else
            {
